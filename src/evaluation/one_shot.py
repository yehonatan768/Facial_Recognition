...
@torch.no_grad()
def evaluate_one_shot_omniglot_paper(
    model,
    cfg: Dict,
    omniglot_root: Path,
    device: torch.device,
    ...
    """
    Strict Koch et al. (2015) Omniglot one-shot protocol:
      - 20-way within-alphabet classification
      - choose an alphabet from evaluation set
      - choose 20 characters from that alphabet
      - choose two drawers: one provides query, other provides support
      - repeat twice per alphabet, for 10 alphabets => 400 trials

    Directory assumption (standard Omniglot):
      root/
    ...
    alphabets = [p for p in omniglot_root.iterdir() if p.is_dir()]
    if len(alphabets) < 10:
        raise ValueError("Need at least 10 alphabets for strict paper protocol.")

    chosen_alphabets = rng.sample(alphabets, k=10)

    correct = 0
    trials = 0
    n_way = 20

    for alphabet_dir in chosen_alphabets:
        chars = [c for c in alphabet_dir.iterdir() if c.is_dir()]
        if len(chars) < n_way:
            continue

        # two repeats per alphabet -> 40 trials (20 each) => 400 total
        for _repeat in range(2):
            chosen_chars = rng.sample(chars, k=n_way)

            # For each character, pick 2 different images (drawer1/query, drawer2/support)
            support_paths = []
            query_paths = []

            for char_dir in chosen_chars:
                imgs = [p for p in char_dir.iterdir() if p.is_file()]
                if len(imgs) < 2:
                    break
                a, b = rng.sample(imgs, k=2)
                query_paths.append(a)
                support_paths.append(b)
            else:
                # run 20 trials: each query compared against the 20 supports
                supports = [transform(load_image_rgb(p)) for p in support_paths]
                s_batch = torch.stack(supports, dim=0).to(device)  # (20,1,H,W)

                for i in range(n_way):
                    q = transform(load_image_rgb(query_paths[i]))
                    q_batch = torch.stack([q] * n_way, dim=0).to(device)

                    if hasattr(model, "score"):
                        p = model.score(q_batch, s_batch).view(-1)
                    else:
                        p, _, _ = model(q_batch, s_batch)
                        p = p.view(-1)
    ...
    acc = correct / float(trials) if trials > 0 else 0.0
    return OneShotResult(accuracy=acc, n_trials=trials, n_way=n_way)
